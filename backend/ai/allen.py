...
# ─────────────────────────────────────────────────────────
# ALLEN'S STORY MAP — destination descriptions
# ─────────────────────────────────────────────────────────

STORYMAP_PROMPT = '''You are Allen, a helpful and engaging local guide for Ottawa, Canada.

The user has just arrived at a destination. Your job is to create a short "story map" — a warm, informative description that helps them understand this place in a clear and interesting way.

Your description should:
- Be 3-5 sentences long
- Include what the place is known for and why it's worth visiting
- Mention practical tips (busy times, nearby landmarks, things to be aware of)
- If the user has shared personal needs (like using a wheelchair, wanting safety, etc.), weave in relevant accessibility or safety information naturally
- Be friendly and welcoming in tone — like a knowledgeable friend showing someone around

Respond with ONLY the description text. No JSON, no markdown, no headings — just the paragraph.'''


def _call_ollama_text(messages: list) -> str:
    """Call Ollama WITHOUT JSON mode — returns plain text."""
    r = req.post(
        f'{OLLAMA_URL}/api/chat',
        json={
            'model': OLLAMA_MODEL,
...
def _call_openai_text(messages: list) -> str:
    """Call OpenAI WITHOUT JSON mode — returns plain text."""
    r = req.post(
        'https://api.openai.com/v1/chat/completions',
        headers={
            'Authorization': f'Bearer {OPENAI_API_KEY}',
        ...
        json={
            ...
            'max_tokens': 400,
            'temperature': 0.6,
            # No response_format — we want natural text
        },
        timeout=15,
    )
    r.raise_for_status()
...
def generate_storymap(destination: str, user_context: str = None) -> Dict:
    """
    Generate a story map description for a destination.
    user_context can include things like "I use a wheelchair" or
    "I'm looking for safe routes" — carried from the Allen chat.
    """
    messages = [{'role': 'system', 'content': STORYMAP_PROMPT}]

    user_msg = f'Destination: {destination}'
    if user_context:
        user_msg += f'\n\nAbout the user: {user_context}'
    ...
    if _ollama_available:
        try:
            content = _call_ollama_text(messages)
            used_backend = 'ollama'
        except Exception as e:
            print(f'[Allen StoryMap] Ollama failed: {e}')

    if content is None and _openai_available:
        try:
            content = _call_openai_text(messages)
            used_backend = 'openai'
        except Exception as e:
            print(f'[Allen StoryMap] OpenAI failed: {e}')

    if content is None:
        raise RuntimeError('No AI backend available for Story Map')
...
# ─────────────────────────────────────────────────────────
# ALLEN'S DIRECTIONS — turn-by-turn walking guidance
# ─────────────────────────────────────────────────────────

DIRECTIONS_PROMPT = '''You are Allen, a friendly walking navigation assistant for Ottawa, Canada.

Given OSRM route step data (maneuver types, street names, distances, durations), your job is to convert them into clear, human-friendly walking directions.

Rules:
- Number each step starting from 1
- Use plain, friendly language anyone can follow
- Include street names when available
- Convert distances to something intuitive (e.g. "about 2 blocks" or "200 metres")
- Convert durations to walking time (e.g. "about 3 minutes")
- If the user has accessibility needs (wheelchair, etc.), mention relevant details like curb cuts or smooth surfaces
- End with a friendly arrival message
- Keep each step to one short sentence

If the user mentioned personal needs, weave in relevant tips naturally (e.g. "This stretch has smooth sidewalks — great for wheels").

Respond with ONLY a JSON array of step objects, nothing else:
[{"step":1,"instruction":"Head north on Bank Street for about 200 metres.","distance_m":200,"duration_s":150},{"step":2,"instruction":"Turn right onto Laurier Avenue.","distance_m":50,"duration_s":35}]

IMPORTANT: Respond with ONLY the JSON array. No markdown, no backticks, no extra text.'''


def generate_directions(steps_data: list, origin: str, destination: str,
                        user_context: str = None) -> Dict:
    """
    Generate human-friendly walking directions from OSRM route steps.
    steps_data is the raw OSRM steps array from route.legs[0].steps.
    """
    # Build a simplified summary for Allen
    step_summary = []
    for s in steps_data:
        ...
        step_summary.append({
            ...
            'street': s.get('name', 'unnamed path'),
            'distance_m': round(s.get('distance', 0)),
            'duration_s': round(s.get('duration', 0)),
        })

    messages = [{'role': 'system', 'content': DIRECTIONS_PROMPT}]

    user_msg = f'Route from: {origin}\nTo: {destination}\n\nSteps data:\n'
    user_msg += json.dumps(step_summary, indent=2)
    if user_context:
    ...
    if _ollama_available:
        try:
            ...
            print(f'[Allen Directions] Ollama failed: {e}')
    ...
    if content is None and _openai_available:
        try:
            ...
            print(f'[Allen Directions] OpenAI failed: {e}')

    if content is None:
        # Fallback: generate basic directions without AI
        return _fallback_directions(steps_data, origin, destination)
    ...
    try:
        ...
        print(f'[Allen Directions] Bad JSON, using fallback. Raw: {cleaned[:200]}')
        return _fallback_directions(steps_data, origin, destination)
...
def _fallback_directions(steps_data: list, origin: str, destination: str) -> Dict:
    """Generate basic directions without AI from raw OSRM steps."""
    TURN_WORDS = {
        'turn': {'left': 'Turn left', 'right': 'Turn right', 'sharp left': 'Take a sharp left',
                 'sharp right': 'Take a sharp right', 'slight left': 'Bear left',
                 'slight right': 'Bear right', '': 'Continue'},
        'depart': {'': 'Start walking'},
        'arrive': {'': 'You have arrived'},
        'continue': {'': 'Continue straight'},
        'new name': {'': 'Continue onto'},
        'end of road': {'left': 'At the end of the road, turn left',
                        'right': 'At the end of the road, turn right', '': 'At the end of the road, continue'},
    ...
    for i, s in enumerate(steps_data):
        ...
        street = s.get('name', '')
        dist = round(s.get('distance', 0))
        dur = round(s.get('duration', 0))

        # Build instruction
        type_map = TURN_WORDS.get(m_type, {'': m_type.replace('_', ' ').capitalize()})
        action = type_map.get(m_mod, type_map.get('', m_type.capitalize()))

        if street and m_type not in ('arrive', 'depart'):
            instruction = f'{action} onto {street}'
        elif street and m_type == 'depart':
            instruction = f'{action} on {street}'
        else:
            instruction = action

        if dist > 0 and m_type != 'arrive':
            if dist < 100:
                instruction += f' ({dist}m)'
            else:
                instruction += f' (about {dist}m, ~{max(1, dur // 60)} min)'

        if m_type == 'arrive':
            instruction = f'You\'ve arrived at {destination}!'
